...
import (
	...
	applogger "task-scheduler/pkg/logger"
...
// MySQLRepository implements TaskRepository using MySQL
...
func (r *MySQLRepository) Create(ctx context.Context, task *domain.Task) error {
	startTime := time.Now()

	applogger.DebugContext(ctx, "Creating task in MySQL",
		zap.String("task_id", task.ID),
		zap.String("name", task.Name))

	model, err := r.domainToModel(task)
	if err != nil {
		applogger.ErrorContext(ctx, "Failed to convert task to model",
			zap.String("task_id", task.ID),
			zap.Error(err))
		metrics.RecordStorageOperation("create", "conversion_error")
		return fmt.Errorf("failed to convert task to model: %w", err)
	}

	if err := r.db.WithContext(ctx).Create(model).Error; err != nil {
		applogger.ErrorContext(ctx, "Failed to create task in MySQL",
			zap.String("task_id", task.ID),
			zap.Error(err))
		metrics.RecordStorageOperation("create", "error")
		return fmt.Errorf("failed to create task: %w", err)
	}

	duration := time.Since(startTime).Seconds()
	metrics.RecordStorageOperation("create", "success")
	metrics.RecordStorageOperationDuration("create", duration)

	applogger.DebugContext(ctx, "Task created successfully in MySQL",
		zap.String("task_id", task.ID),
...
func (r *MySQLRepository) Get(ctx context.Context, taskID string) (*domain.Task, error) {
	var model taskModel
...
func (r *MySQLRepository) Update(ctx context.Context, task *domain.Task) error {
	model, err := r.domainToModel(task)
	if err != nil {
		return fmt.Errorf("failed to convert task to model: %w", err)
	}
...
func (r *MySQLRepository) Delete(ctx context.Context, taskID string) error {
...
func (r *MySQLRepository) List(ctx context.Context, filter *domain.TaskFilter) ([]*domain.Task, error) {
...
func (r *MySQLRepository) GetSubTasks(ctx context.Context, parentID string) ([]*domain.Task, error) {
	var models []taskModel
...
func (r *MySQLRepository) domainToModel(task *domain.Task) (*taskModel, error) {
	model := &taskModel{
		ID:               task.ID,
		Name:             task.Name,
...
func (r *MySQLRepository) modelToDomain(model *taskModel) (*domain.Task, error) {
	task := &domain.Task{
		ID:               model.ID,
		Name:             model.Name,
